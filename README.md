...
> **RAG query-back is parked**, not built into this version — see [ADR 015](decisions/015-park-rag-query-back.md).
> It carried more infrastructure (eval harness, full pipeline tracing, prompt caching) than an unmeasured,
> low-frequency personal feature needs. Code is preserved and restorable in [`archive/rag/`](archive/rag/README.md);
> it comes back once real usage data says it's worth it.
...
parked — see [ADR 015](decisions/015-park-rag-query-back.md) — so the
pipeline currently ends at the web digest.
...
| Web framework | FastAPI + Jinja2 | Web digest, `/healthz` — no build step |
...
| Vector store | pgvector (Neon Postgres) | Powers L2 near-dup detection; single DB, no extra infra |
...
RAG query answers (`claude-sonnet-4-6`) are parked along with the rest of the
RAG feature — see [ADR 015](decisions/015-park-rag-query-back.md).
...
> LangSmith full-pipeline tracing was scoped entirely to the RAG query path
> and is parked along with it — see [ADR 015](decisions/015-park-rag-query-back.md)
> and [`archive/rag/docs/langsmith-tracing.md`](archive/rag/docs/langsmith-tracing.md).
...
├── archive/rag/              # parked RAG query-back — retriever, responder, eval harness, restorable (ADR 015)
...
8. **Verify**: `GET /healthz`, `GET /` (digest)

Environment variables — same set as local dev (`OPENROUTER_API_KEY`,
`NOMIC_API_KEY`, `DATABASE_URL`, `DEDUP_THRESHOLD`), no Telegram vars.
Prefer Secret Manager over plaintext `--set-env-vars` for anything sensitive.

> `--allow-unauthenticated` is used since this is a single-user tool with no
> auth system — meaning `/` is public on the Cloud Run URL. See the "Known
> gap" note in [`docs/gcp-deployment.md`](docs/gcp-deployment.md) if that
> matters to you.
...
| Service | Plan | $/month |
|---|---|---|
| OpenRouter (Gemini Flash + Haiku) | Pay-as-you-go | ~$2–3 |
...
| [`archive/rag/docs/eval-harness.md`](archive/rag/docs/eval-harness.md) | RAGAS 4-metric eval harness (parked with RAG) — how to run, interpret scores |
| [`archive/rag/docs/langsmith-tracing.md`](archive/rag/docs/langsmith-tracing.md) | LangSmith RAG tracing (parked with RAG) — setup, span architecture |
